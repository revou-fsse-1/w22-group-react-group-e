...
export default function BlogPostPage({ data }: { data: SanityDocument }) {
  ...
  return (
    <div className="justify-center p-10 m-10">
      <h1 className="p-10 text-4xl font-bold text-center bg-[#EEF6F4] text-emerald-950">{title}</h1>
      {mainImage && (
        ...
        <div>
          {body.map((block: any, index: number) => {
            if (block._type === "block" && block.style === "h2") {
              return <h2 className="p-5 mb-3 text-2xl font-semibold" key={index}>{block.children[0].text}</h2>;
            } else if (block._type === "block") {
              return <p className="justify-between p-3 mb-2 text-lg" key={index}>{block.children[0].text}</p>;
            }
            return null;
          })}
        </div>
